...
MicroProfile OpenAPI specification builds upon the widely recognized OpenAPI Specification (OAS) and leverages annotations from the Jakarta Restful Web Services specification. The primary focus of MicroProfile OpenAPI is on defining REST APIs that utilize JSON within the context of HTTP.

The specification aims to provide a uniform way of describing APIs so that it is both human-readable as well as machine-readable.  It facilitates the creation of APIs that are consistent, well-documented, and easily consumable by both humans and machines. 
...
Developer can quickly generate documentation for their microservices using MicroProfile OpenAPI. The documentation includes information on what services are provided, how to invoke them, and what data types are used. It generates comprehensive metadata about services, ensuring interoperability across diverse platforms and tools. In addition, the documentation can generate client code to access the web services.
...
* *API Mocking*: Simplifies the mocking of APIs for testing and development purposes.
...
To document a Jakata RESTful Webservices using MicroProfile OpenAPI, we need to annotate the resource classes and methods with the OpenAPI annotation. 
...
| @OpenAPIDefinition
| Provides metadata about the entire API. It can inclde information such as the title, description, version, terms of service, and contact information.
...
== Summary
We have learned most of the essential concepts related to the MicroProfile Framework. In the next chapter, we will see how to secure a RESTful Web Service using the JWT Propagation feature of the MicroProfile.
