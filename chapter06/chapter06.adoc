...
The HealthCheckResponseBuilder abstract class
...
In your Kubernetes deployment configuration, define the readiness probe to use the /health/ready endpoint, which MicroProfile exposes for readiness checks. Kubernetes will automatically handle this as a combined startup and readiness probe if no explicit startup probe is defined.
readinessProbe:
...
If you specifically want to use Kubernetes' startup probe feature, you can configure it similarly in your Kubernetes deployment YAML.
This approach lets you leverage MicroProfile Health in a cloud-native environment effectively, ensuring your application is only considered ready after successful completion of essential startup checks.
